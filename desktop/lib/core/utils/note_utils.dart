...
/// to "Untitled".
///
/// Identical to the mobile app's `lib/utils/note.dart` so both clients
/// derive identical titles from the same canonical AppFlowy JSON.
class NoteUtils {
  ...
  static void _collectLines(Map<String, dynamic> node, List<String> out) {
    final delta =
        node['delta'] ?? node['data']?['delta'] ?? node['attributes']?['delta'];
    if (delta is List) {
