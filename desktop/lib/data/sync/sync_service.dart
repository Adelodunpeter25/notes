  ...
  Future<void> syncData(String userId) async {
    ...
    try {
      ...
      for (final d in deletedList) {
        ...
        if (entityId == null || entityType == null || deletedAt == null) {
          continue;
        }

        if (entityType == 'note') {
          await (db.update(db.notes)..where((t) => t.id.equals(entityId)))
              .write(NotesCompanion(deletedAt: Value(deletedAt)));
          await db.deleteNoteFts(entityId);
        } else if (entityType == 'folder') {
          await (db.update(db.notes)..where((t) => t.folderId.equals(entityId)))
              .write(const NotesCompanion(folderId: Value(null)));
          await (db.delete(db.folders)..where((t) => t.id.equals(entityId)))
              .go();
        }
      }
    } catch (e, stackTrace) {
