...
import '../sync/sync_op_recorder.dart';
...
class NoteService {
  ...
  Future<void> autoDeleteEmptyNotes(String userId) async {
    ...
    for (final note in potentialEmpty) {
      // Match the desktop behavior: a note whose title is blank or the
      // literal "Untitled" placeholder, and whose body has no text, is an
      // abandoned empty note. ("Untitled" is only ever stored as a display
      // placeholder — the editor derives real titles from content.)
      final title =
          note.title.trim().toLowerCase();
      final isTitleEmpty = title.isEmpty || title == 'untitled';
      if (isTitleEmpty && NoteUtils.isContentEmpty(note.content)) {
