...
class NoteDao extends DatabaseAccessor<AppDatabase> with _$NoteDaoMixin {
  ...
  /// Reactive count of non-deleted notes for a user.
    ...
    return customSelect(
      ...
      return {
        for (final row in rows)
          row.read<String>('folder_id'): row.read<int>('cnt'),
      };
    });
  }
  ...
  /// Used when a folder is deleted so its notes don't become orphans.
  ...
  /// Prefix-matches every whitespace-separated term (`apple* AND pie*`).
...
class FolderDao extends DatabaseAccessor<AppDatabase> with _$FolderDaoMixin {
  ...
  Future<int> deleteFolder(Folder entry) =>
      (delete(folders)..where((t) => t.id.equals(entry.id))).go();
}
