...
@DriftDatabase(
  tables: [Users, Folders, Notes, SyncOps],
  daos: [NoteDao, FolderDao, SyncOpDao],
)
class AppDatabase extends _$AppDatabase {
  /// [executor] is injectable so tests can run against an in-memory database.
      ...
      onUpgrade: (m, from, to) async {
        ...
        if (from < 3) {
          await customStatement(
              'DROP TABLE IF EXISTS ${syncOps.actualTableName}');
          await m.createTable(syncOps);
        }
        if (from < 4) {
          await customStatement('''
...
LazyDatabase _openConnection() {
  return LazyDatabase(() async {
    ...
    final file = File(p.join(dbFolder.path, 'note_desktop_db.sqlite'));
