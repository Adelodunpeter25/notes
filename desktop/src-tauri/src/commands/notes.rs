...
pub fn list_notes(
    ...
    let mut query = "SELECT id, user_id, folder_id, title, content, is_pinned, created_at, updated_at 
                     FROM notes 
...
pub fn get_note(app: AppHandle, id: String) -> Result<Note> {
    ...
    let mut stmt = db
        .prepare("SELECT id, user_id, folder_id, title, content, is_pinned, created_at, updated_at 
                  FROM notes 
